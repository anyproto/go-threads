...
import (
	...
	"errors"
	"fmt"
	"github.com/textileio/go-threads/mongo/test"
	"log"
	"math/rand"
	"net"
	"os"
...
func TestClient_Verify(t *testing.T) {
	...
	t.Run("test collection verify", func(t *testing.T) {
		...
		person.ID = ids[0]
		person.Age = 51
		err = client.Verify(context.Background(), id, collectionName, Instances{person})
		if err == nil {
			t.Fatalf("failed to verify instance")
		}
		person.Age = 50
		err = client.Verify(context.Background(), id, collectionName, Instances{person})
		if err != nil {
...
func TestClient_WriteTransaction(t *testing.T) {
	...
	t.Run("test write transaction", func(t *testing.T) {
		...
		err = txn.Verify(existingPerson)
		if err != nil {
			t.Fatalf("failed to verify in write txn: %v", err)
		}
